    ...
    <article className='w-full max-w-5xl overflow-hidden rounded-3xl border border-stone-200 bg-stone-100/95 p-6 shadow-sm'>
      <div className='grid gap-6 md:grid-cols-[340px_1fr] md:items-center'>
        <div className='relative mx-auto h-65 w-full max-w-85'>
          <div className='absolute left-0 top-7 w-[45%] -rotate-2 rounded-xl bg-white p-2 shadow-lg'>
            <img src={current.imageFile} alt={current.title || entry.title} className='h-45 w-full rounded-lg object-cover' />
          </div>

          <div className='absolute right-2 top-0 w-[53%] rotate-2 rounded-xl bg-white p-2 shadow-lg'>
            <img src={next.imageFile} alt={next.title || entry.title} className='h-52.5 w-full rounded-lg object-cover' />
          </div>
        </div>

        <div className='space-y-3 text-left'>
          <h3 className='text-4xl font-bold leading-tight text-rose-900'>{entry.title}</h3>
          <p className='text-lg font-semibold text-stone-600'>{entry.month}</p>
          <p className='max-w-xl text-xl leading-relaxed text-stone-700'>{entry.desc}</p>
          <p className='text-sm text-stone-600'>{current.desc || 'No description provided.'}</p>
          {entry.location && <p className='text-sm text-stone-600'>Location: {entry.location}</p>}

          <div className='flex items-center gap-3 pt-2'>
            <button
              type='button'
              onClick={showPrevious}
              className='rounded-full bg-rose-900 px-3 py-1 text-sm font-semibold text-white transition hover:bg-rose-800'
